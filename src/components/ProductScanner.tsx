...
export function ProductScanner({ onProductAdded }: ProductScannerProps) {
    ...
    startTransition(async () => {
      try {
        ...
        if (result && result.name && result.expiryDate) {
          ...
          setScanFeedback({ message: `Added: ${result.name}`, timestamp: Date.now() });
          toast({
              title: "Product Added",
              description: `${result.name} (exp: ${result.expiryDate}) has been added.`,
          });
        } else {
            toast({
              variant: 'destructive',
              title: "Scan Failed",
              description: "Could not extract all details. Please try again.",
          });
        }
      } catch (error) {
  ...
  return (
    <Card className="shadow-lg bg-card/80 backdrop-blur-sm">
      <CardHeader>
        <CardTitle className="text-2xl font-headline">Smart Scanner</CardTitle>
        <CardDescription>Point your camera at a barcode. We'll find the product and expiry date.</CardDescription>
      </CardHeader>
      <CardContent>
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center p-4">
                 {!isDetectorSupported ? (
                    <Alert variant="destructive" className="w-full">
                        ...
                        <AlertDescription>Barcode scanning is not supported on this browser. Please try Chrome on Android or a desktop computer.</AlertDescription>
                    </Alert>
                 ) : (
                    <Alert variant="destructive" className="w-full">
                        <Camera className="h-4 w-4" />
                        <AlertTitle>Camera Error</AlertTitle>
                        <AlertDescription>Could not access camera. Please check permissions.</AlertDescription>
                    </Alert>
                 )}
              </div>
            )}
